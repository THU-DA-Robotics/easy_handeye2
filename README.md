...
Launch the state and TF publisher on the franka control PC:
```shell
ssh ...

ros2 launch franka_bringup franka.launch.py robot_ip:=192.168.52.3 use_rviz:=true load_gripper:=true
```

Launch the camera:
```shell
ros2 launch object_detection kinect_bringup.py
```

Launch the aruco and easy-eye-hand (Notice: must have a rqt/rviz to subscribe the aruco/results/image):
```
ros2 launch easy_handeye2 realsense_ur_calib.launch.py
```

Then, manually move the robot and take samples.
